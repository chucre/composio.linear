...
from .my_issues import MyIssues
...
class Client(BaseClient):
    def my_issues(self, **kwargs: Any) -> MyIssues:
        query = gql(
            """
            query MyIssues {
        ...
        response = self.execute(
            query=query, operation_name="MyIssues", variables=variables, **kwargs
        )
        data = self.get_data(response)
        return MyIssues.model_validate(data)
